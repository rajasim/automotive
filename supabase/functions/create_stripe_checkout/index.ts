...
async function createCheckoutSession(
  ...
  if (enrollmentData) {
    orderData.customer_email = enrollmentData.email;
    orderData.customer_name = enrollmentData.name;
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: {
      order_id: order.id,
      user_id: userId || "",
      enrollment_data: enrollmentData ? JSON.stringify(enrollmentData) : "",
    },
  });
